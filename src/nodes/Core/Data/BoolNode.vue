...
function validateBool() {
  const lower = props.data.value.toLowerCase();
  if (lower === 'true' || lower === 'false')
  {
    updateNodeData(props.id, { value: lower });
  }
  else 
  {
    props.data.value = 'false'
    updateNodeData(props.id, 'false');
  }
...
<template>
    ...
    >
        <template #body>
            <div class="bool-node-body">
                <input
                v-model="localValue"
                type="text"
                placeholder="false"
                class="bool-node-input"
                @keyup.enter="validateBool"
                @blur="validateBool"
                />
            </div>
        </template>
        
        <template #output-Bool="{handle}">
          <Handle
              :id="`source-${handle.id}`"
              type="source"
...
.custom-node.bool-node .node-body {
  padding-left: 10px;
  padding-right: 10px;
}
