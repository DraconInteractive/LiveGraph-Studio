...
const outputs = [
  { id: 'Float', dataType: 'float' }
]

props.data.inputs = inputs
props.data.outputs = outputs
props.data.class = 'float-node'
...
function validateFloat() {
  const parsed = parseFloat(props.data.value)
  if (isNaN(parsed)) {
    props.data.value = '0'
...
<template>
    ...
    >
        <template #body>
            <div class="float-node-body">
                <input
                v-model="localValue"
                type="text"
                placeholder="0.0"
                class="float-node-input"
                @keyup.enter="validateFloat"
                @blur="validateFloat"
                />
            </div>
        </template>
        
        <template #output-Float="{handle}">
          <Handle
              :id="`source-${handle.id}`"
              type="source"
...
.custom-node.float-node .node-body {
  padding-left: 10px;
  padding-right: 10px;
}
